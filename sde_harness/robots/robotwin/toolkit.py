...
exposing a small RPent Toolkit:
...
class RobotTwinEnvAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        observation_encoder: Callable[[dict[str, Any], str], dict[str, Any]] | None = None,
    ) -> None:
...
class RobotTwinToolkit(Toolkit):
    ...
    def execute_ee(self, action: list[float], repeat: int = 1) -> dict[str, Any]:
        arr = np.asarray(action, dtype=np.float32).reshape(-1)
        if arr.size != 16:
            raise ValueError(f"RoboTwin EE action must contain 16 values, got {arr.size}")
        count = max(1, min(int(repeat), 8))
        applied = 0
        for _ in range(count):
            self.env.take_action(arr, action_type="ee")
            self._record(arr, action_type="ee", source="robotwin_execute_ee")
            applied += 1
            if self.env.done():
                break
        return {
            "applied_steps": applied,
            **self._payload(include_images=True, last_action=arr),
        }
    ...
    def _predict_action(self) -> np.ndarray:
        ...
        arr = arr.reshape(-1)
        if arr.size != 16:
            raise ValueError(f"Hy-VLA RoboTwin action must contain 16 values, got {arr.size}")
        return arr

    def vla_step(self, steps: int = 1) -> dict[str, Any]:
        count = max(1, min(int(steps), 32))
        applied = 0
        last_action: np.ndarray | None = None
        for _ in range(count):
            if self.env.done():
                break
            last_action = self._predict_action()
            self.env.take_action(last_action, action_type="ee")
            self._record(last_action, action_type="ee", source="robotwin_vla_step")
            applied += 1
        return {
            "applied_steps": applied,
            **self._payload(include_images=True, last_action=last_action),
        }
...
__all__ = ["RobotTwinEnvAdapter", "RobotTwinToolkit"]
