...
from hy_harness.utils.templates import substitute
...
class BaseTool:
    ...
    def add_tool(
        ...
        Args:
            ...
            handler: Callable invoked with the tool's input kwargs; returns
                a result dict.
        """
        self._tools[tool_name] = (tool_description, tool_function)

    def _register_common_tools(self) -> None:
        """Register the file/IO tools shared by every run."""
        from hy_harness.tools import common

        for tool_description in common.TOOLS_DESCRIPTION:
            name = tool_description["name"]
            self.add_tool(name, tool_description, common.TOOL_HANDLERS[name])
    ...
    def get_tools_description(self) -> list[dict[str, Any]]:
        ...
        return substitute([desp for desp, _ in self._tools.values()])
