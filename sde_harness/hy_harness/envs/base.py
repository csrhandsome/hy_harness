...
the ``hy_harness`` package); an env is resolved by importing ``robots.<name>``. The
``EnvSpec`` / ``RunConfig`` dataclasses themselves live in
:mod:`hy_harness.envs` so planners and envs share the same contract types without
crossing module layers. ``EnvSpec`` also carries the three runner hooks
(``add_cli_args`` / ``parse_config`` / ``init_runtime``) that keep
``hy_harness/cli/main.py`` env-agnostic.
"""
...
from hy_harness.envs.env_spec import EnvSpec
...
# ``hy_harness`` distribution. Ensure the repo root is importable so ``robots.<name>``
...
def get_tools(name: str, **kwargs) -> BaseTool:
